...
public class EasyBot implements Bot{
    ...
    public EasyBot(Color clr, Map m) {
        myColor = clr;
        map = m;
        money = 10;
        moneyTimer = 0;
            ...
            if(mode == SOLDIER) {
                ...
                for(int soldierLevel = 1; soldierLevel <= 5; soldierLevel++) {
                    ...
                    while(count < countLimit) {
                            ...
                            if((row+col) % 2 == 0) {
                                if(!map.tileColors[row][col].equals(myColor) && map.besideColor(row, col, myColor) && map.water[row][col] == false) { 
                                    if(map.getDefense(row, col) < soldierLevel) {
                                        if(money >= 10*soldierLevel) {
                                            map.objects[row][col] = new Soldier(soldierLevel, (int)(30*moveDelay));
                                            map.tileColors[row][col] = myColor;
                                            money -= 10*soldierLevel;
                                            cooldown = actionDelay;
                                            mode = selectMode();
                                            return;
                    ...
                    while(count < countLimit) {
                            ...
                            if((row+col) % 2 == 0) {
                                if(map.besideColor(row, col, myColor) && map.water[row][col]) { 
                                        int shipLevel = (int)(5*Math.random());
