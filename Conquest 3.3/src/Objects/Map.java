...
public class Map {
            ...
            for(int i = 0; i < 40; i++) {
                ...
                if(row < 10 || row > 30 || col < 10 || col > 30) {
                    if(terrain[row][col] <= 0) {
                        ...
                        for(int r = 0; r < 40; r++) { 
                            for(int c = 0; c < 40; c++) {
                                if(Map.distance(r,c,row,col) <= 2 && terrain[r][c] > 0) nope = true;
                            }
                        }
                        if(!nope) {
                            for(int r = 0; r < 40; r++) { 
                                for(int c = 0; c < 40; c++) {
                                    ...
                                    if(Map.distance(r,c,row,col) == 2 && Math.random() < 0.5) terrain[r][c]+=2;
                                }
                            }
                        }
                    }
    ...
    */
    ...
    //Returns the income without soldiers, towers, or ships
    public int getRawIncome(Color myColor) {
        int income = 0;
        for(int row = 0; row < 40; row++) {
            for(int col = 0; col < 40; col++) {
                if((row+col) % 2 == 0) {
